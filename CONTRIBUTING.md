...
| Factor | GitHub issue or Candidate PR first | A judgment rule may become a built-in or third-party reusable Factor |
...
5. Built-in Factor and scanner changes enter `packs/session-signal/` or `packages/runtime/` after maintainer routing and high-risk review.
6. Third-party executable packs use an independent Repo and immutable Release before registry publication.
...
- Add or revise one Factor Candidate, registry reference, or Factor authoring rule.
