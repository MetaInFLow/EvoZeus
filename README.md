...
EvoZeus 也定义一种新的软件范式：**Skill Driven Software（SDS）**。在 SDS 中，软件行为由 code、scenario skill、factor、rule、report 和 runtime 共同驱动。代码提供可执行底座，Skill 提供场景化行动方式，Factor 提供判断机制，Rule 提供可复用决策，Report 把证据和下一步动作展示给人。
...
Verdict（裁决）不停留在观点层。它必须落成 Artifact：
...
这个仓库当前提供协议和共创面；CLI release 尚未进入当前承诺范围。

| Surface | File |
| --- | --- |
| Agent 入口 | [SKILL.md](SKILL.md) |
| 项目理念 | [VISION.md](VISION.md) |
| Skill Driven Software | [docs/concepts/skill-driven-software.zh-CN.md](docs/concepts/skill-driven-software.zh-CN.md) |
| Skill Matrix | [skills/index/SKILL.md](skills/index/SKILL.md) |
| Skill Package Manifest | [docs/reference/skill-package-manifest.md](docs/reference/skill-package-manifest.md) |
...
| Factor 协议 | [docs/reference/factor-analysis-protocol.md](docs/reference/factor-analysis-protocol.md) |
...
README 只保留项目用途、启动语、资产范围、运行原则和贡献入口。

完整阅读路径、开发记录、协议参考和治理规则见 [docs/README.md](docs/README.md)。
...
| Scenario Skills | `skills/index/SKILL.md`、场景化 skill packages、下载授权 gate |
| Local Runtime | `.evozeus/` 本地状态、SQLite registry、Markdown/JSON report |
| Factor Runtime + Library | analysis framework、factor manifest、inspect、enable、rollback |
