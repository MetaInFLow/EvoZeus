...
QUALITY_EVIDENCE_LIMIT = 6
FACTOR_EVIDENCE_RECORD_LIMIT = 4
EVIDENCE_SNIPPET_CHARS = 520
...
FACTOR_EVIDENCE_LABELS = {
    "official.user-input-sentiment": "用户输入情绪",
    "official.task-completion": "任务完成判断",
    "official.repeated-request": "重复请求",
    "official.tool-failure-frequency": "工具失败",
    "official.session-resource-usage": "资源使用",
    "official.key-sentence-trends": "关键句",
    "official.semantic-phrase-clusters": "语义短句簇",
}
SENTIMENT_EVIDENCE_TERMS = {
    "correction_request": (
        "不对",
        "理解错",
        "方向不对",
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .skill-candidate-table {{ table-layout: fixed; min-width: 1640px; }}
    .skill-candidate-table th:nth-child(1), .skill-candidate-table td:nth-child(1) {{ width: 130px; }}
    ...
    .skill-candidate-table th:nth-child(4), .skill-candidate-table td:nth-child(4) {{ width: 280px; }}
    .skill-candidate-table th:nth-child(5), .skill-candidate-table td:nth-child(5) {{ width: 170px; }}
    .skill-candidate-table th:nth-child(6), .skill-candidate-table td:nth-child(6) {{ width: 540px; }}
    .skill-candidate-table th:nth-child(7), .skill-candidate-table td:nth-child(7) {{ width: 220px; }}
...
def _quality_signal_for(
    ...
    semantic_cluster_count = int(metrics["semantic_cluster_count"])
    if semantic_cluster_count:
        diagnostics.append(f"识别到 {semantic_cluster_count} 个语义短句簇")
    ...
    factor_evidence = _factor_evidence_details(results, events, compact=True)
...
def _quality_metrics(result_by_factor: dict[str, FactorResult]) -> dict[str, Any]:
    ...
    semantic_phrases = result_by_factor.get("official.semantic-phrase-clusters")
    ...
    return {
        ...
        "semantic_cluster_count": int(_score_value(semantic_phrases, "semantic_phrase_cluster_count", 0.0)),
    }
...
def _quality_label(metrics: dict[str, Any], score: float) -> str:
    ...
    repeated_count = int(metrics["repeated_request_count"])
    failure_count = int(metrics["tool_failure_count"])
    if strong_user_problem or negative_count:
        return "problem_skill_candidate"
    if repeated_count:
        return "repeat_skill_candidate"
    if completion in {"blocked", "not_completed"}:
...
def _render_quality_method_card(by_label: dict[str, int], candidate_count: int) -> str:
    items = [
        ...
        ("表达模式", "`key-sentence-trends` 和 `semantic-phrase-clusters` 提供触发语、关键动作、输出对象和语义变体。"),
    ]
    body = "".join(
...
def _render_quality_table(title: str, description: str, signals: list[dict[str, Any]]) -> str:
    ...
    chunks = [
        ...
        "<th>结论</th><th>建议沉淀成</th><th>session</th><th>为什么</th><th>factor 指标</th><th>Factor 命中 input</th><th>下一步</th>",
        "</tr></thead><tbody>",
    ]
    for signal in rows:
        risks = "; ".join(str(item) for item in signal.get("risks", [])[:3]) or "无明显风险"
        reasons = "; ".join(str(item) for item in signal.get("reasons", [])[:3])
        fallback_evidence = "; ".join(str(item) for item in signal.get("evidence_previews", [])[:2]) or _refs_preview(
            signal.get("evidence_refs", [])
        )
        evidence = _render_factor_evidence(
...
def _render_session(
    ...
    if results:
        chunks.append(_render_session_results(results, events))
...
def _render_session_results(results: list[FactorResult], events: list[SessionEventRecord]) -> str:
    ...
    for result in sorted(results, key=lambda item: item.factor_id):
        factor_evidence = _render_factor_evidence(
            _factor_evidence_details([result], events, compact=False),
            fallback="",
        )
        chunks.append(
...
def _quality_evidence_refs(results: list[FactorResult]) -> list[dict[str, str]]:
    ...
    priority = {
        ...
        "official.semantic-phrase-clusters": 6,
    }
...
def _factor_evidence_details(
    ...
    *,
    ...
    priority = {
        "official.user-input-sentiment": 0,
        "official.repeated-request": 1,
        "official.task-completion": 2,
        "official.tool-failure-frequency": 3,
        "official.key-sentence-trends": 4,
        "official.semantic-phrase-clusters": 5,
        "official.session-resource-usage": 6,
    }
    for result in sorted(results, key=lambda item: priority.get(item.factor_id, 99)):
        if result.factor_id == "official.user-input-sentiment":
            details.extend(_sentiment_evidence_details(result, events_by_id, compact=compact))
        elif result.factor_id == "official.repeated-request":
            details.extend(_repeated_request_evidence_details(result, events_by_id))
        elif result.factor_id == "official.task-completion":
            details.extend(_task_completion_evidence_details(result, events_by_id, compact=compact))
        elif result.factor_id == "official.tool-failure-frequency":
            details.extend(_tool_failure_evidence_details(result, events_by_id))
        else:
            details.extend(_generic_factor_evidence_details(result, events_by_id, compact=compact))
        if compact and len(details) >= QUALITY_EVIDENCE_LIMIT:
            return details[:QUALITY_EVIDENCE_LIMIT]
    return details[:QUALITY_EVIDENCE_LIMIT if compact else QUALITY_EVIDENCE_LIMIT]


def _sentiment_evidence_details(
    result: FactorResult,
    events_by_id: dict[str, SessionEventRecord],
    *,
    compact: bool,
) -> list[dict[str, Any]]:
    records = _dataset_records(result, "user_input_sentiment")
    if compact:
        records = [record for record in records if str(record.get("sentiment_kind") or "") != "neutral_request"]
    records = sorted(records, key=_sentiment_record_rank)[:FACTOR_EVIDENCE_RECORD_LIMIT]
    details: list[dict[str, Any]] = []
    for record in records:
        event_id = str(record.get("event_id") or "")
        event = events_by_id.get(event_id)
        kind = str(record.get("sentiment_kind") or "unknown")
        text = _sentiment_record_text(event, record)
        item = _event_evidence_item(
            event,
            label="命中 input",
        ...
        details.append(
            {
                "factor_id": result.factor_id,
                "label": FACTOR_EVIDENCE_LABELS.get(result.factor_id, result.factor_id),
                "tag": f"user_sentiment={kind}",
                "summary": _join_meta(
                    [
                        f"confidence={_format_number(_float_value(record.get('confidence'), 0.0))}",
                        str(record.get("nearest_example") or ""),
                    ]
                ),
                "items": [item],
            }
...
def _repeated_request_evidence_details(
    result: FactorResult,
    events_by_id: dict[str, SessionEventRecord],
) -> list[dict[str, Any]]:
    records = _dataset_records(result, "repeated_request_events")[:FACTOR_EVIDENCE_RECORD_LIMIT]
    details: list[dict[str, Any]] = []
    for record in records:
        first_event_id = str(record.get("first_event_id") or "")
        repeat_event_id = str(record.get("repeat_event_id") or record.get("event_id") or "")
        details.append(
            {
                "factor_id": result.factor_id,
                "label": FACTOR_EVIDENCE_LABELS.get(result.factor_id, result.factor_id),
                "tag": "loop=repeated-request",
                "summary": _join_meta(
                    [
                        str(record.get("chain_id") or ""),
                        f"similarity={_format_number(_float_value(record.get('similarity_score'), 0.0))}",
                        str(record.get("request_signature") or ""),
                    ]
                ),
                "items": [
                    _event_evidence_item(
                        events_by_id.get(first_event_id),
                        label="首次请求",
                        fallback_event_id=first_event_id,
                        text_override=str(record.get("first_input_text") or ""),
                    ),
                    _event_evidence_item(
                        events_by_id.get(repeat_event_id),
                        label="重复请求",
                        fallback_event_id=repeat_event_id,
                        text_override=str(record.get("repeat_input_text") or ""),
                    ),
                ],
            }
        )
...
def _task_completion_evidence_details(
    result: FactorResult,
    events_by_id: dict[str, SessionEventRecord],
    *,
    compact: bool,
) -> list[dict[str, Any]]:
    records = _dataset_records(result, "task_completion_verdict")
    if compact:
        records = [
            record
            for record in records
            if str(record.get("verdict") or "") in {"blocked", "not_completed", "unknown"}
            or str(record.get("evidence_event_id") or "")
        ]
    details: list[dict[str, Any]] = []
    for record in records[:FACTOR_EVIDENCE_RECORD_LIMIT]:
        event_id = str(record.get("evidence_event_id") or "")
        details.append(
            {
                "factor_id": result.factor_id,
                "label": FACTOR_EVIDENCE_LABELS.get(result.factor_id, result.factor_id),
                "tag": f"task_completion={str(record.get('verdict') or 'unknown')}",
                "summary": f"completion_score={_format_number(_float_value(record.get('completion_score'), 0.0))}",
                "items": [
                    _event_evidence_item(
                        events_by_id.get(event_id),
                        label="完成判断证据",
                        fallback_event_id=event_id,
                        text_override=str(record.get("evidence_preview") or ""),
                    )
                ],
            }
        )
...
def _tool_failure_evidence_details(
    result: FactorResult,
    events_by_id: dict[str, SessionEventRecord],
) -> list[dict[str, Any]]:
    records = _dataset_records(result, "tool_failure_frequency")[:FACTOR_EVIDENCE_RECORD_LIMIT]
    details: list[dict[str, Any]] = []
    for record in records:
        event_ids = record.get("sample_event_ids")
        if not isinstance(event_ids, list):
            event_ids = []
        items = [
            _event_evidence_item(
                events_by_id.get(str(event_id)),
                label="失败事件",
                fallback_event_id=str(event_id),
            )
        ...
        details.append(
            {
                "factor_id": result.factor_id,
                "label": FACTOR_EVIDENCE_LABELS.get(result.factor_id, result.factor_id),
                "tag": f"tool_failure={str(record.get('tool_name') or 'unknown')}",
                "summary": f"count={_format_number(_float_value(record.get('count'), 0.0))}",
                "items": items,
            }
        )
    return details
...
def _generic_factor_evidence_details(
    result: FactorResult,
    events_by_id: dict[str, SessionEventRecord],
    *,
    compact: bool,
) -> list[dict[str, Any]]:
    if result.status != "matched" and compact:
        return []
    refs = [ref for ref in result.evidence_refs if isinstance(ref, dict)]
    items: list[dict[str, Any]] = []
    for ref in refs[:FACTOR_EVIDENCE_RECORD_LIMIT]:
        event_id = str(ref.get("ref_id") or "")
        items.append(
            _event_evidence_item(
                events_by_id.get(event_id),
                label=str(ref.get("kind") or "evidence"),
                fallback_event_id=event_id,
            )
        )
    if not items:
    ...
    return [
        {
            "factor_id": result.factor_id,
            "label": FACTOR_EVIDENCE_LABELS.get(result.factor_id, result.factor_id),
            "tag": _factor_tag_summary(result),
            "summary": "evidence_refs joined to session_events",
            "items": items,
        }
    ]
...
def _event_evidence_item(
    ...
    *,
    ...
    text = text_override.strip() if text_override else ""
    if event is not None and not text:
        text = event.content or event.tool_result_preview or ""
    source = ""
    role = ""
    event_index = ""
    if event is not None:
        ...
        source = f"{event.source_ref}:{event.source_line}" if event.source_line else event.source_ref
    if not text:
        text = "event content not materialized"
    return {
        "label": label,
        "event_id": event.event_id if event is not None else fallback_event_id,
        "event_index": event_index,
        "role": role,
        "text": _truncate_text(text, EVIDENCE_SNIPPET_CHARS),
        "source": source,
    }


def _render_factor_evidence(details: object, *, fallback: str) -> str:
    if not isinstance(details, list) or not details:
        return f'<div class="evidence-empty">{_esc(fallback)}</div>' if fallback else ""
    chunks = ['<div class="evidence-stack">']
    for detail in details:
        if not isinstance(detail, dict):
            continue
        chunks.append(
            '<div class="factor-evidence">'
            '<div class="factor-evidence-title">'
            f'<span>{_esc(str(detail.get("label") or detail.get("factor_id") or "factor"))}</span>'
            f'<span class="role">{_esc(str(detail.get("tag") or ""))}</span>'
            f'<code>{_esc(str(detail.get("factor_id") or ""))}</code>'
            "</div>"
        )
        summary = str(detail.get("summary") or "")
        if summary:
            chunks.append(f'<div class="factor-evidence-summary">{_esc(summary)}</div>')
        items = detail.get("items")
        if isinstance(items, list):
            for item in items:
                if not isinstance(item, dict):
                    continue
                chunks.append(_render_evidence_item(item))
        chunks.append("</div>")
    chunks.append("</div>")
    return "\n".join(chunks)
...
def _render_evidence_item(item: dict[str, Any]) -> str:
    event_index = str(item.get("event_index") or "")
    index = f"#{event_index}" if event_index else "#?"
    role = str(item.get("role") or "unknown")
    source = str(item.get("source") or "")
    source_html = f'<div class="evidence-source">source <code>{_esc(source)}</code></div>' if source else ""
    return (
        '<div class="evidence-item">'
        '<div class="evidence-item-head">'
        f'<span class="role">{_esc(str(item.get("label") or "evidence"))}</span>'
        f'<span>{_esc(index)}</span>'
        f'<span>{_esc(role)}</span>'
        f'<code>{_esc(str(item.get("event_id") or ""))}</code>'
        "</div>"
        f'<div class="evidence-snippet">{_esc(str(item.get("text") or ""))}</div>'
        f"{source_html}"
        "</div>"
    )


def _sentiment_record_rank(record: dict[str, Any]) -> tuple[int, float]:
    kind = str(record.get("sentiment_kind") or "")
    priority = {
        "dissatisfaction": 0,
    ...
    confidence = _float_value(record.get("confidence"), 0.0)
    return priority, -confidence


def _sentiment_record_text(event: SessionEventRecord | None, record: dict[str, Any]) -> str:
    for key in ("matched_excerpt", "input_text", "evidence_preview", "text_preview"):
    ...
    text = event.content or event.tool_result_preview or ""
    kind = str(record.get("sentiment_kind") or "")
    sentence = _sentence_with_terms(text, SENTIMENT_EVIDENCE_TERMS.get(kind, ()))
    return sentence or text


def _sentence_with_terms(value: str, terms: tuple[str, ...]) -> str:
    if not value or not terms:
        return ""
    normalized_terms = [term.lower() for term in terms if term]
    for part in _split_evidence_sentences(value):
        lowered = part.lower()
...
def _factor_tag_summary(result: FactorResult) -> str:
    values = [f"{tag.get('type', '')}={tag.get('value', '')}" for tag in result.tags if isinstance(tag, dict)]
    return ", ".join(value for value in values if value and value != "=") or result.status


def _join_meta(values: list[str]) -> str:
    return " / ".join(value for value in values if value)


def _evidence_search(details: object) -> str:
    ...
    for detail in details:
        if not isinstance(detail, dict):
            continue
        values.extend(str(detail.get(key) or "") for key in ("factor_id", "label", "tag", "summary"))
        items = detail.get("items")
        if isinstance(items, list):
            for item in items:
                if isinstance(item, dict):
                    values.extend(str(item.get(key) or "") for key in ("event_id", "role", "text", "source"))
    return " ".join(values)
...
def _quality_search(signal: dict[str, Any] | None) -> str:
    ...
    parts = [
        ...
        _evidence_search(signal.get("factor_evidence")),
    ]
