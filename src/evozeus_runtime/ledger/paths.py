...
class RuntimePaths:
    workspace_root: Path
    explicit_state_root: Path | None = None

    @classmethod
    def for_workspace(cls, workspace_root: Path) -> RuntimePaths:
        configured = os.environ.get("EVOZEUS_RUNTIME_STATE_ROOT")
        return cls(
            workspace_root=workspace_root.expanduser().resolve(),
            explicit_state_root=Path(configured).expanduser().resolve() if configured else None,
        )

    @property
    def state_root(self) -> Path:
        return self.explicit_state_root or self.workspace_root / ".evozeus"
