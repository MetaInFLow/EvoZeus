...
class CodexScanner(SessionScanner):
    ...
    def discover_message_refs(self, ref: SessionRef) -> list[SessionMessageRef]:
        ...
        for raw_line_index, record in _iter_jsonl_records(ref.source_path):
            ...
            if message_id is None:
                continue
            message_index += 1
            message_metadata = _message_ref_metadata_from_record(record, ref.metadata)
    ...
    def iter_events(self, ref: SessionRef) -> Iterator[SessionEvent]:
        ...
        source_fingerprint = str(ref.metadata.get("source_fingerprint") or _source_fingerprint(ref.source_path))
        event_index = 0
        pending_event: SessionEvent | None = None
        for index, record in _iter_jsonl_records(ref.source_path):
            ...
            if event is not None:
                ...
                event = _with_locator(
                    ...
                    session_id=session_id,
                    session_metadata=ref.metadata,
                )
                if pending_event is not None and _is_mirrored_message_event(pending_event, event):
                    event_index -= 1
                    yield _mark_mirrored_user_event(pending_event, event)
                    pending_event = None
                    continue
                if pending_event is not None:
                    yield _mark_unmirrored_user_event(pending_event, ref.metadata)
                pending_event = event
        if pending_event is not None:
            yield _mark_unmirrored_user_event(pending_event, ref.metadata)
    ...
    def load(self, ref: SessionRef) -> SessionEnvelope:
        ...
        return SessionEnvelope(
            ...
            metadata={
                ...
                "session_thread_source": str(ref.metadata.get("session_thread_source") or ""),
                "session_source_kind": str(ref.metadata.get("session_source_kind") or ""),
                "subagent_parent_thread_id": str(ref.metadata.get("subagent_parent_thread_id") or ""),
                "subagent_agent_nickname": str(ref.metadata.get("subagent_agent_nickname") or ""),
                "subagent_agent_role": str(ref.metadata.get("subagent_agent_role") or ""),
                "subagent_depth": str(ref.metadata.get("subagent_depth") or ""),
...
def _message_ref_metadata_from_record(record: dict[str, Any], session_metadata: dict[str, str] | None = None) -> dict[str, str]:
    ...
    if isinstance(payload, dict):
        ...
        if event is not None:
            metadata.update(_channel_metadata(event, session_metadata))
        return metadata
    ...
    metadata.update(_channel_metadata(_event_from_flat_payload(0, record), session_metadata))
    return metadata
...
def _tool_result_payload(payload: dict[str, Any]) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for key in ("name", "arguments", "input", "output", "status", "call_id"):
        if key in payload:
...
def _with_locator(
    ...
    *,
    ...
    content_hash = _content_hash(event.content)
    channel_metadata = _channel_metadata(event, session_metadata)
    metadata.update(
        {
            "provider": "codex",
            "scanner_id": SCANNER_ID,
            "scanner_version": SCANNER_VERSION,
            **_event_session_lineage_metadata(session_metadata),
            **channel_metadata,
            "source_ref": str(source_path),
            "source_fingerprint": source_fingerprint,
            "content_hash": content_hash,
...
def _channel_metadata(event: SessionEvent, session_metadata: dict[str, str] | None = None) -> dict[str, str]:
    raw_role = event.role
    ...
    message_scope = _message_scope(event, content_kind, session_metadata)
    return {
        "raw_role": raw_role,
        "chat_role": chat_role,
...
def _factor_text_for_preview(event: SessionEvent, content_kind: str) -> str:
    if content_kind == "real_user_message":
        return _message_body_for_dedupe(event.content)
    return event.content


def _message_scope(event: SessionEvent, content_kind: str, session_metadata: dict[str, str] | None = None) -> str:
    if event.role != "user":
        return ""
    text = event.content.strip()
    if not text:
        return "empty"
    session_scope = _session_message_scope(session_metadata)
    if session_scope:
    ...
    if content_kind == "codex_context" or REQUEST_MARKER_RE.search(text):
...
def _mark_mirrored_user_event(primary: SessionEvent, mirror: SessionEvent) -> SessionEvent:
    if primary.role != "user" or mirror.role != "user":
        return primary
    metadata = dict(primary.metadata)
    metadata["codex_user_origin"] = "event_msg_mirror" if _is_event_user_message(mirror) else "response_item_mirror"
    return primary.model_copy(update={"metadata": metadata})


def _mark_unmirrored_user_event(event: SessionEvent, session_metadata: dict[str, str] | None) -> SessionEvent:
    if event.role != "user":
        return event
    metadata = dict(event.metadata)
    if _is_event_user_message(event):
        metadata["codex_user_origin"] = "event_msg"
        return event.model_copy(update={"metadata": metadata})
    if not _is_response_user_message(event):
        return event
    if _is_new_user_thread(session_metadata):
        metadata["codex_user_origin"] = "synthetic_context"
        if metadata.get("factor_channel") == "user_input":
            metadata.update(
                {
                    "content_kind": "codex_context",
                    "factor_channel": "context",
                    "chat_role": "context",
                    "message_scope": "context_wrapper",
                    "factor_text_preview": _preview(event.content),
                }
            )
        return event.model_copy(update={"metadata": metadata})
    return event
...
def _is_event_user_message(event: SessionEvent) -> bool:
    return (
        event.role == "user"
        and event.metadata.get("codex_record_type") == "event_msg"
        and event.metadata.get("codex_event_type") in {"user_message", "user"}
    )


def _is_response_user_message(event: SessionEvent) -> bool:
    return (
        event.role == "user"
        and event.metadata.get("codex_record_type") == "response_item"
        and event.metadata.get("codex_event_type") == "message"
    )


def _is_new_user_thread(session_metadata: dict[str, str] | None) -> bool:
    if not session_metadata:
        return False
    return str(session_metadata.get("session_thread_source") or "") == "user"
...
def _message_body_for_dedupe(content: str) -> str:
    body = _extract_user_request(content).replace("\\n", "\n")
    for pattern in DROP_BLOCK_PATTERNS:
        body = pattern.sub(" ", body)
    body = " ".join(body.split())
    if "data:image" in body.lower() or "image_url" in body.lower():
        body = re.sub(r"data:image/[A-Za-z0-9.+-]+;base64,[A-Za-z0-9+/=]+", "data:image", body)
    return body.strip()
...
def _session_lineage_metadata(session_meta: dict[str, Any] | None) -> dict[str, str]:
    payload = session_meta.get("payload") if isinstance(session_meta, dict) else None
    ...
    if isinstance(source, dict) and isinstance(source.get("subagent"), dict):
        ...
        raw_spawn = subagent.get("thread_spawn") if isinstance(subagent, dict) else None
        if isinstance(raw_spawn, dict):
            spawn = raw_spawn
    elif source:
        source_kind = str(source)
    parent_thread_id = str(payload.get("parent_thread_id") or payload.get("forked_from_id") or spawn.get("parent_thread_id") or "")
    return {
        "session_thread_source": str(payload.get("thread_source") or ("subagent" if source_kind == "subagent" else "")),
        "session_source_kind": source_kind,
        "subagent_parent_thread_id": parent_thread_id,
        "subagent_agent_nickname": str(payload.get("agent_nickname") or spawn.get("agent_nickname") or ""),
        "subagent_agent_role": str(payload.get("agent_role") or spawn.get("agent_role") or ""),
        "subagent_depth": str(spawn.get("depth") or ""),
    }


def _event_session_lineage_metadata(session_metadata: dict[str, str] | None) -> dict[str, str]:
    if not session_metadata:
        return {}
    keys = (
    ...
    return {key: str(session_metadata.get(key) or "") for key in keys if session_metadata.get(key)}


def _session_message_scope(session_metadata: dict[str, str] | None) -> str:
    if not session_metadata:
        return ""
    thread_source = str(session_metadata.get("session_thread_source") or "")
    source_kind = str(session_metadata.get("session_source_kind") or "")
    if thread_source == "automation":
        return "automation"
    if thread_source == "subagent" or source_kind == "subagent" or session_metadata.get("subagent_parent_thread_id"):
        return "delegated_task"
    return ""
