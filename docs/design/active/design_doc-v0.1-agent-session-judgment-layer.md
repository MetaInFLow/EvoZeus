...
EvoZeus 不是 Agent 打分器，不是 benchmark，不是普通报告工具。
...
Skill 不是边界。Skill 只是 Verdict 之一。
...
这个结构对齐 openLifeOS 的“协议 + schema + scripts + output”思路，但宙斯的核心资产不是人格资产，而是 Agent Session 判例。
