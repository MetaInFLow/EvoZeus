...
| Lesson 候选 | `🧙 EvoZeus · 捕捉到一条 Lesson｜<脱敏摘要>｜拟记录到：<目标 Repo 或 local-only 位置> · <记录载体>｜影响范围：<受影响对象>｜写入边界：<本次确认创建的记录；排除动作>。要按此记录吗？` | 业务结果完成、记录落点与授权边界均已明确后发现可复用改进 |
...
3. Lesson 提示永远出现在用户任务结果之后；提示必须明确目标 Repo 或 local-only 位置、记录载体、影响范围和写入边界，用户确认前不持久化。
...
8. 目标位置、记录载体或写入边界无法确定时，不显示通用的 Lesson 记录确认问题。
